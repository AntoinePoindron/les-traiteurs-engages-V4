(function () {
  ...
  function createLineHTML(section, data) {
    ...
    return '<div class="flex flex-wrap items-start gap-2 p-3 rounded-lg border border-cream-200 bg-cream-50" id="' + id + '" data-section="' + section + '">' +
      '<div class="flex-1 min-w-[200px]">' +
        ...
        '<input type="text" value="' + desc.replace(/"/g, '&quot;') + '" class="line-desc w-full border border-cream-200 rounded px-2 py-1.5 text-sm text-navy focus:ring-1 focus:ring-terracotta focus:border-terracotta">' +
      '</div>' +
      '<div class="w-20">' +
        '<label class="block text-xs text-navy-300 mb-1">Qte</label>' +
        '<input type="number" value="' + qty + '" min="0" step="1" class="line-qty w-full border border-cream-200 rounded px-2 py-1.5 text-sm text-navy text-right focus:ring-1 focus:ring-terracotta focus:border-terracotta">' +
      '</div>' +
      '<div class="w-28">' +
        '<label class="block text-xs text-navy-300 mb-1">PU HT (&euro;)</label>' +
        '<input type="number" value="' + price + '" min="0" step="0.01" class="line-price w-full border border-cream-200 rounded px-2 py-1.5 text-sm text-navy text-right focus:ring-1 focus:ring-terracotta focus:border-terracotta">' +
      '</div>' +
      '<div class="w-24">' +
        '<label class="block text-xs text-navy-300 mb-1">TVA</label>' +
        '<select class="line-tva w-full border border-cream-200 rounded px-2 py-1.5 text-sm text-navy focus:ring-1 focus:ring-terracotta focus:border-terracotta">' + tvaOptions + '</select>' +
      '</div>' +
      ...
      '</div>' +
      '<div class="pt-6">' +
        '<button type="button" data-action="remove-line" data-target="' + id + '" class="text-coral-red-400 hover:text-coral-red-500 transition-colors">' +
  ...
  // Delegated listeners for dynamic add/remove + recompute on input changes.
  document.addEventListener('click', function (ev) {
    var addBtn = ev.target.closest('[data-action="add-line"]');
    if (addBtn) {
      addLine(addBtn.dataset.section);
      return;
    }
    var rmBtn = ev.target.closest('[data-action="remove-line"]');
    if (rmBtn) {
      removeLine(rmBtn.dataset.target);
    }
  });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (cfg) {
      ...
      try { INITIAL_LINES = JSON.parse(cfg.dataset.initialLines || '[]'); } catch (e) { INITIAL_LINES = []; }
    }
